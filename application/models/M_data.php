...
class M_data extends CI_Model
{
    ...
    public function get_index_orwhere($condition, $where, $where2, $table)
    {
        return $this->db->order_by($condition, 'desc')->where($where)->or_where($where2)->get($table);
    }
    ...
    {
        $this->db->select('h.id AS id,id_pengguna,id_penjual,alamat,d.id AS id_detil,nama,status,id_produk,pr.nama_produk,pr.satuan,pr.stok,jumlah,d.harga,h.ongkir');
